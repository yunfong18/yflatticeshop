...
class WCFM_Shipping_Easypost_Admin {
    ...
    function wf_admin_notice() {
        ...
        $wfeasypostmsg = wc_clean($_GET["wfeasypostmsg"]);
    ...
    function wf_easypost_account_metabox_content() {
        ...
        try {
            ...
            $plugin_dir = WP_PLUGIN_DIR . '/easypost-woocommerce-shipping/';
    ...
    function wf_easypost_add_funds() {
        ...
        $query_string = explode('|', base64_decode(wc_clean($_GET['wf_easypost_add_funds'])));
        $post_id = $query_string[0];
        $easypost_authenticator = $query_string[1];
        $control_total = $query_string[2];
        $easypost_topup_amount = $_GET['wf_easypost_topup_amount'] ? wc_clean($_GET['wf_easypost_topup_amount']) : '';
    ...
    function wf_easypost_shipment_confirm() {
        ...
        $query_string = explode('|', base64_decode(wc_clean($_GET['wf_easypost_shipment_confirm'])));
        $post_id = $query_string[1];
        $wf_easypost_selected_service = isset($_GET['wf_easypost_service']) ? wc_clean($_GET['wf_easypost_service']) : '';
        update_post_meta($post_id, 'wf_easypost_selected_service', $wf_easypost_selected_service);

        $selected_flatrate_box = isset($_GET['wf_easypost_flatrate_box']) ? wc_clean($_GET['wf_easypost_flatrate_box']) : '';
	...
	function wf_easypost_generate_packages(){
		...
		$post_id = base64_decode(wc_clean($_GET['wf_easypost_generate_packages']));
    ...
    function wf_easypost_void_shipment() {
        ...
        $post_id = base64_decode(wc_clean($_GET['wf_easypost_void_shipment']));
