				...
				<div class="address rgt">
				  ...
				  <?php do_action( 'before_wcfmmp_store_header_info', $store_user->get_id() ); ?>
					...
					<?php if( $address && ( $store_info['store_hide_address'] == 'no' ) && $WCFM->wcfm_vendor_support->wcfm_vendor_has_capability( $store_user->get_id(), 'vendor_address' ) ) { ?>
						<p class="<?php echo $store_address_info_class; ?>"><i class="wcfmfa fa-map-marker" aria-hidden="true"></i>
						  <?php if( apply_filters( 'wcfmmp_is_allow_address_map_linked', true ) ) { ?>
						    <a href="https://maps.google.com/?q=<?php echo rawurlencode( $address ); ?>&z=16" target="_blank"><span><?php echo $address; ?></span></a>
						  <?php } else { ?>
								<?php echo $address; ?>
							<?php } ?>
						</p>
